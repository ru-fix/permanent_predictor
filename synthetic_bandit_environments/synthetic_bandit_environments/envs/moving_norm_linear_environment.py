...
class MovingNormLinearEnvironment(gym.Env):

    def __init__(
            self, amount_arms=4, context_size=10, amount_contexts=100,
            arm_range=(0, 1), context_range=(0, 1),
            arm_error_params=(0, 0.05), smoof_steps=1, seed=None
    ):
        super(MovingNormLinearEnvironment, self).__init__()
    ...
    def __calculate_rewards(self):
        ...
        self.arm_errors = (1 - self.lambdas[self.lambda_index]) * self.start_arm_errors \
                                + self.lambdas[self.lambda_index] * self.finish_arm_errors
        rewards = np.dot(self.contexts[self.index], self.arms.T) + self.arm_errors
